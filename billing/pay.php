...
if(isset($_GET['id'])){
    $id = $_GET['id'];
    $query = "SELECT p_id FROM reports WHERE id=$id AND paid=b'0' AND pay_time IS NULL";
    $res = mysqli_query($conn,$query);
    if(mysqli_affected_rows($conn)>0){
        $payment_details = array();
        $mode = $_POST["mode"];
        $p_id = mysqli_fetch_assoc($res)["p_id"];
		...
		$date=date("Y/m/d");
        $query = "UPDATE reports SET paid='1', pay_time='$date', payment_details='$pd' WHERE id=$id";
        mysqli_query($conn,$query);
        $donation = $_POST['donation'];
        if($donation!=0){
            $query = "SELECT amount FROM donations WHERE p_id=$p_id";
            $donation;
            $res = mysqli_query($conn,$query);
            if(mysqli_affected_rows($conn)>0){
                $donation += mysqli_fetch_assoc($res)['amount'];
                $query = "UPDATE donations SET amount=$donation WHERE p_id=$p_id";
            }else{
                $query = "INSERT INTO donations(p_id, type, amount) VALUES($p_id, 'patient', $donation)";
            }
            mysqli_query($conn,$query);
        }
    }else{
        ...
        <div class="main-content">
            <div class="container-fluid">
                 ...
                 <div class="row">
                    ...
                    <div class="col-md-6"><iframe id="preview" name="preview" width="100%" height="900px" src="bill.php?id=<?php echo $_GET['id']?>"></iframe></div>
                    <div class="col-md-3"></div>
                 </div>
