...
if(is_not_empty($_REQUEST['from']) && is_not_empty($_REQUEST['to'])){
  ...
  $query = "SELECT r.id,r.doc_id,r.p_id,r.base_treatment,r.treatments,p.fname,p.lname,r.pay_time FROM patients p, reports r WHERE r.p_id=p.id AND r.pay_time>='$from' AND r.pay_time<='$to' AND paid=1";
}else{
...
if(isset($_GET['action']) && $_GET['action']=="download"){
  ...
  $sheet->setCellValue('E5', 'Treated By');
  ...
  $sheet->setCellValue('B3', $_REQUEST['to']);
    ...
    while($row = mysqli_fetch_assoc($result)) {
      ...
      $sheet->setCellValue('E'.$c, $rw['name']);
      $sheet->setCellValue('F'.$c, $data['amount']);
      $t=$t+$data['amount'];
      $c++;
    }
