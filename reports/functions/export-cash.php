...
if(is_not_empty($_REQUEST['from']) && is_not_empty($_REQUEST['to'])){
  ...
  $query = "SELECT * FROM cashbook WHERE date>='$from' AND date<='$to' ";
}else{
...
if(isset($_GET['action']) && $_GET['action']=="download"){
  ...
  $sheet->setCellValue('C5', 'Opening Balance');
  $sheet->setCellValue('D5', 'Credit');
  $sheet->setCellValue('E5', 'Debit');
  $sheet->setCellValue('F5', 'Balance');
  $sheet->setCellValue('A1', 'Report Type: ');
  $sheet->setCellValue('B1', 'Cashbook Report');
  ...
  $sheet->setCellValue('B3', $_REQUEST['to']);
    ...
    while($row = mysqli_fetch_assoc($result)) {
      ...
      $sheet->setCellValue('C'.$c, $row['opening']);
      $sheet->setCellValue('D'.$c, $row['credit']);
      $sheet->setCellValue('E'.$c, $row['debit']);
      $sheet->setCellValue('F'.$c, $row['balance']);
  ...
  header('Content-Disposition: attachment;filename="Cashbook.xlsx"');
  ...
  if(mysqli_affected_rows($conn)>0){
    $url = "functions/export-cash.php?action=download&from=".$_REQUEST['from']."&to=".$_REQUEST['to'];
    $resp['url'] = $url;
    $success = true;
  }
